...
model.to(device)
        ...
        # half disabled train bug · Issue #99 · ultralytics/yolov5
    ...
    seen = 0
    names = ['wheat']#model.names #if hasattr(model, 'names') #else model.module.names
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        img = Img To Wholesale and Shopping(device)
        ...
        for si, pred in enumerate(output):
            ...
            if save_json:
                ...
                #print('Path(paths[si]).stem.split(_)[-1]',Path(paths[si]).stem.split('_')[-1])
                #print('int(Path(paths[si]).stem.split(_)[-1])',int(Path(paths[si]).stem.split('_')[-1]))
